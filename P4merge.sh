...
#Declare variable and iniate it - Get the no diff
...
STR1=$(p4.exe diff2 -qOd //vsd-ptcs/ems/8x/dev_br/EMS8X-3183_emd1/... //vsd-ptcs/ems/8x/main/...)
...
then
   ...
   read -p "Do you want to begin the Merge Procedure? [y/N] " response
   response=${response,,}
   if [[ "$response" =~ ^(yes|y)$ ]]
   then
